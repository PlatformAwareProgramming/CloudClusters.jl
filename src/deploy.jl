...
default_exename() = "julia"   # read from a CloudClousters.toml config file
default_exeflags() = "--optimize=2"
default_tunnel() = true                    # read from a CloudClousters.toml config file
...
function cluster_deploy(contract_handle, cluster_handle)
    ...
    cluster_provider = cluster_features[:provider]
...
function cluster_deploy(cluster_provider, cluster_type::Type{<:ManagerWorkers}, cluster_handle, cluster_features, instance_type)
    ...
    #ip_master, nw, user_id, key_name = deploy_cluster(cluster_provider, cluster_type, CreateMode, cluster_handle, cluster_features, instance_type_master, instance_type_worker)
    ip_master = Dict()
    ip_master[:public_ip] = "127.0.0.1"
    ip_master[:private_ip] = "127.0.0.1"
    nw = 4
    user_id = "heron"
    key_name = "/home/heron/hpc-shelf-credential.pem"
    ...
    master_id = addprocs(["$user_id@$(ip_master[:public_ip])"]; sshflags="-i $key_name", exename=exename_master, exeflags=exeflags_master, tunnel=tunnel #=, ident=ident, connect_idents=connect_idents=#)
...
function cluster_deploy(cluster_provider, cluster_type::Type{<:PeerWorkers}, cluster_handle, cluster_features, instance_type)
    ...
    #ips, user_id, key_name = deploy_cluster(cluster_provider, cluster_type, CreateMode, cluster_handle, cluster_features, instance_type)
    ...
    peer_ids = addprocs(["$user_id@$(ip[:public_ip])" for ip in ips], sshflags="-i $key_name", exename=exename, exeflags=exeflags, tunnel=tunnel#=, ident=ident, connect_idents=connect_idents=#)
